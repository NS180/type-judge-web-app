...
import { getRandomSentence, getRandomMainSentence, MAIN_SENTENCES, LONG_SENTENCES, SPEED_MODE_SENTENCES, ZEN_MODE_SENTENCES, CHAOS_MODE_SENTENCES, MEME_MODE_SENTENCES } from '@/lib/sentences';
...
export function TypingTest() {
  ...
  const levelConfig = {
    easy: { label: 'WARM-UP', description: 'Short prompt. Learn the rhythm.', target: '20+ WPM', length: 1 },
    medium: { label: 'MAIN CHARACTER', description: 'A proper paragraph for a real read.', target: '35+ WPM', length: 3 },
    hard: { label: 'BOSS FIGHT', description: 'An extended passage. Accuracy under pressure.', target: '55+ WPM', length: 5 },
  } as const;
  const activeLevel = levelConfig[level as keyof typeof levelConfig];
  ...
  const getSentenceForMode = (nextMode = mode, nextLevel = level) => {
    if (nextMode === 'classic') {
      ...
      return Array.from({ length: levelConfig[nextLevel as keyof typeof levelConfig].length }, () => getRandomSentence(LONG_SENTENCES)).join(' ');
    }
    ...
    const count = nextLevel === 'easy' ? 1 : nextLevel === 'medium' ? 2 : 4;
    return Array.from({ length: count }, () => getRandomSentence(pool)).join(' ');
  };
      ...
      <div className="max-w-4xl mx-auto">
        {isComplete && testResult ? (
          ...
          <div>
            ...
            <div className="level-status" aria-live="polite">
